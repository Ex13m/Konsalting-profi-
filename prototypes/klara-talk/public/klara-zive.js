...
(function () {
  ...
  function kresli(t) {
    ...
    const kyv = kyvOd ? prikyvnuti(t - kyvOd) : 0;
    ...
    const naklon = (Math.sin(t / 2300) * 0.9 + Math.sin(t / 900) * 0.35) * zivost
                 ;
    // Shared CSS wrapper animates canvas and greeting consistently.
    ctx.globalAlpha = 1;
    ctx.translate(W / 2 + (reducedMotion.matches ? 0 : naklon), H);
    ctx.scale(1 + (reducedMotion.matches ? 0 : dech), 1 + (reducedMotion.matches ? 0 : dech));
    ctx.translate(-W / 2, -H);
